...
def predict_hierarchy(cells: Sequence[int], lmax: int = 0, num_ranks: int = 1, bottom_extent: int = 4) -> dict:
    """
    The pressure-multigrid ladder a Flow builds on `cells` FINEST cells per axis at tree depth `lmax` over `num_ranks` ranks (docs/amr_mg_depth.md §6.2, §6.7) — a pre-flight tool. A pure function: it builds no mesh and needs no MPI; it re-runs the ladder rule on the ORB a freshly constructed DistributedOctree would produce (not a partition after `DistributedOctree.rebalance` / `Flow.rebalance_mpi`). Below the root brick the ladder halves the grid while every rank's block stays even; where it stops above `bottom_extent` cells per axis on more than one rank the level MOVES (a stage): onto fewer ranks by a sibling merge or a repartition when the Flow's stage policy is on (docs/amr_mg_depth.md WO4b), where the ladder lifts again, else gathered onto every rank (the replicated tail) and continued there; the coarsest level is then solved by damped-Jacobi sweeps if it has at most `bottom_extent` cells per axis, else by the agglomerated GraphAMG-PCG bottom. The prediction is that of the default `Flow.set_pressure_bottom('auto')`.

    Returns a dict, levels finest first: `extent` the level's global grid in CELLS per axis (a count, not a length), `cells` its total cell count as if the mesh were uniform (so it equals `Flow.diagnostics.pressure_mg_levels` leaf for leaf only for a uniform mesh at num_ranks=1), `kind` ('octree' at or above the root brick, 'lifted' below it in place, 'tail' / 'sibling' / 'repartition' on a stage's continued ladder -- a stage's first level is the SAME grid as the level above it, moved rather than coarsened, exactly as `pressure_mg_levels` lists it), `num_levels`, `num_in_place` (levels above the first stage), `stages` (the stage kinds, outermost first), `tail` (bool: a replicated tail engages) and `bottom` ('jacobi' or 'amg', suffixed '+tail' / '+sibling' / '+repartition' per stage, innermost first) — the spelling `Flow.diagnostics.pressure_mg_bottom` reports.
...
class Flow:
    ...
    def set_pressure_pcg(self, on: bool) -> None:
        """
        The pressure-solve DRIVER of the aperture projection. on=True (THE DEFAULT) is MG-preconditioned conjugate gradients: one V-cycle of the pressure multigrid preconditions CG, the fluid-mean component of the residual is removed every iteration (an aperture cut-cell right-hand side is incompatible by exactly that component), and the solve stops at set_pressure_tolerance or at step()'s / project()'s `pres_iters`. on=False is the bounded stationary V-cycle: exactly `pres_iters` V-cycles, with no tolerance test and no mean removal, so on cut-cell geometry its residual STALLS at the incompatible component instead of converging. Both solve the same pressure equation, so where both converge they give the same projected velocity to the solve tolerance; the choice moves cost and robustness, not the answer. Leave it on: on=False exists to A/B the driver on a compatible problem. It has NO effect under the ghost projection -- the default scheme -- whose nonsymmetric operator is always solved by MG-preconditioned BiCGStab; it acts with set_ghost_projection(False) or where the default fell back to the aperture projection. flow's `Solver.set_pressure_pcg` is the same selector, but flow has no stationary driver (on=False raises there) and takes the cap and tolerance as its max_iter / rtol arguments. Takes effect at the next step()/project(); on a DISTRIBUTED Flow pass the same value on every rank.
        """
...
class FlowDiagnostics:
    ...
    def pressure_mg_levels(self) -> list[int]:
        """
        Leaf count of every pressure-multigrid level, finest first (docs/amr_mg_depth.md §6.7), as a list of int; empty before set_solid, which is where the hierarchy is built. Levels below the root brick are LIFTED levels — the same octree with its root halved — so a uniform mesh has a real hierarchy rather than a single level. Under MPI the in-place levels are THIS RANK's counts, the levels of a replicated tail (appended last) are GLOBAL counts, identical on every rank, and the levels of a sibling-merge or repartition stage are this rank's counts on its target block (0 on a rank that owns none). Check it against `peclet.amr.predict_hierarchy` (level count, and leaf for leaf on a uniform mesh at np=1), never against a literal.
        """

    @property
    def pressure_mg_bottom(self) -> str:
        """
        What solves the coarsest pressure level of the hierarchy AS BUILT: 'jacobi' (60 damped-Jacobi sweeps, effectively exact at <= 4 cells per axis: the slowest mode falls by 8e-9) or 'amg' (the agglomerated GraphAMG-PCG solve), with the suffix '+tail' when the coarsest level was gathered onto every rank by the replicated stage, '+sibling' / '+repartition' when a level moved onto fewer ranks (one suffix per stage, innermost first; docs/amr_mg_depth.md §6.5-§6.7, WO4b). Which one runs follows `Flow.set_pressure_bottom` (default 'auto': the exact bottom engages only where the ladder ran out above `Flow.pressure_bottom_extent` cells per axis). 'jacobi' before set_solid.
        """
