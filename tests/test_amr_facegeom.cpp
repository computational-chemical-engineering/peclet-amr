// Device collocated FACE-GEOMETRY assembly (peclet::amr::assembleFaceGeom) must reproduce
...
void run() {
  ...
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.start), down(dg.start)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.nbr), down(dg.nbr)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.axis), down(dg.axis)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.dir), down(dg.dir)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.alphaArea), down(dg.alphaArea)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.rawArea), down(dg.rawArea)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.dist), down(dg.dist)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.alpha), down(dg.alpha)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.upupI), down(dg.upupI)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.upupJ), down(dg.upupJ)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.invVol), down(dg.invVol)), 0);
  PECLET_AMR_CHECK_EQ(mismatch(down(hg.fluid), down(dg.fluid)), 0);
}
