// Distributed dynamic adapt (peclet::amr::distributedAdapt): solution-adaptive (re)meshing
...
void run() {
  ...
  PECLET_AMR_CHECK(gnw == self.local().numLeaves());  // same global leaf count
  int mism = 0;
  ...
  PECLET_AMR_CHECK(std::fabs(gm - m0) < 1e-9 * std::fabs(m0));  // conservative remap
  unsigned minL = 99;
  ...
  PECLET_AMR_CHECK(gminL < (long)kLmax);  // refined below the base level
}
