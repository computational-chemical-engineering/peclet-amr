...
`CMAKE_PREFIX_PATH` points at) and optionally on **MPI** (`PECLET_AMR_MPI`, default ON — the distributed
octree / flow and the Python module need it). The suite-wide contract is in `../docs/`
...
cmake -S . -B build_q -DCMAKE_PREFIX_PATH=/home/frankp/Codes/suite/extern/install/host-openmp \
      -DPECLET_AMR_MPI=ON -DPECLET_AMR_BUILD_TESTS=ON -DMPIEXEC_EXECUTABLE=/usr/bin/mpirun
...
**Counts** (host-openmp): `-DPECLET_AMR_MPI=ON` registers **92** C++ ctests (28 single-rank + 16
distributed binaries × np = 1, 2, 4, 8) + 5 `bench` (four `study_amr_*` + `bench_amr_flow`) + 3
`python` (`python_amr`, `python_amr_np2`, `python_state_hash`) = 100; `-DPECLET_AMR_MPI=OFF` registers
28 + 4 `bench` (no Python module). The Kokkos+MPI battery was 92 + 2 Python ctests in core's
`build_rel_k` / `build_rel_py` before the move and reproduces there test for test.
...
`python/state_hash_reference.json` is the committed reference (recorded on the pre-relocation core
build and reproduced byte-for-byte here). Any structural change must leave every hash identical;
a numerics change re-records the reference in its own commit and says so.

CI (`.github/workflows/ci.yml`): gcc + clang Release (MPI, tests, Python on gcc), gcc Debug, and
`PECLET_AMR_MPI=OFF`, each with `core` (at `PECLET_CORE_REF`, `main` until the core release that
...
target `peclet_amr` / `peclet::amr`; options `PECLET_AMR_MPI`, `PECLET_AMR_BUILD_PYTHON`,
`PECLET_AMR_BUILD_TESTS`, `PECLET_CUDA_RUNTIME_WHEEL`; `cmake --install` exports
