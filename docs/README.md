...
| [amr_mg_depth.md](amr_mg_depth.md) | **The pressure multigrid below the root brick** (ROADMAP C1): a lifted root, the lockstep distributed ladder, the replicated-tail stage, the agglomerated `GraphAMG`-PCG bottom, `predict_pressure_hierarchy` and the `Flow.diagnostics` read-outs. §11 carries the measured answers to its open questions (§11.4: the bottom extent stays 4; §11.10: how to read an iteration count). |
